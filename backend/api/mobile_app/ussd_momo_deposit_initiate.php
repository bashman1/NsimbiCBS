...
if ($sql["Status"] == "OK") {
    ...
    $stmt = $itemn->InsertMmTrxnLogUssd($data->my_phone, (int)$data->counter, $data->sid, isset($sql['TransactionReference']) ? $sql['TransactionReference'] : "", $data->reason, $stcode == "0" ? "SUCCEEDED" : ($stcode == "1" ? "PENDING" : "FAILED"), $data->amount, $mes,$ext_ref, $data->phone,'DEPOSIT');
} else {
    if ($sql['StatusCode'] == "--") {
