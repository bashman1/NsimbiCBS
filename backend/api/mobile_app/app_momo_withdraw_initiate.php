...
if ($sql["Status"] == "OK") {
    ...
    $stmt = $itemn->InsertMmTrxnLog($data->school_name, $data->school_acno, $data->user_id, isset($sql['TransactionReference']) ? $sql['TransactionReference'] : "", $data->reason, $stcode == "0" ? "SUCCEEDED" : ($stcode == "1" ? "PENDING" : "FAILED"), $data->amount, $mes, $data->branch,$data->phone,$ext_ref);
} else {
    if ($sql['StatusCode'] == "--") {
