...
$stmt = $item->createdById == '' ? $item->getBranchClients(preg_replace("/\s+/", "", $_GET['q'])) : $item->getBankClients(preg_replace("/\s+/", "", $_GET['q']));
...
if ($itemCount > 0) {
    ...
    while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
        ...
        $record = array(
            ...
            "gender" => $gender,
            "acc_balance" => number_format($acc_balance),
            "loan_wallet" => number_format($loan_wallet),
            "tot_balance" => number_format($loan_wallet + $acc_balance),
