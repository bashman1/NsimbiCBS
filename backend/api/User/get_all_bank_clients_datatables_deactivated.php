...
try {
    $records = $client->getBankClientsDatatableDeactivated()->fetchAll(PDO::FETCH_ASSOC);
    if ($client->bank) {
        $recordsTotal =  count($client->getBankClientsDeactivated()->fetchAll(PDO::FETCH_ASSOC));
    } else {
        $recordsTotal =  count($client->getBranchClientsDeactivated()->fetchAll(PDO::FETCH_ASSOC));
    }
