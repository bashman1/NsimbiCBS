...
$client->filter_saving_officer  = @$_REQUEST['agent'];
...
if($_GET['bank']){
    ...
    $client->bank = $client->getBankId(@$_GET['branch']);
}
...
try {
    ...
    if ($client->bank) {
        $recordsTotal =  count($client->getBankClients()->fetchAll(PDO::FETCH_ASSOC));
    } else {
        $recordsTotal =  count($client->getBranchClients()->fetchAll(PDO::FETCH_ASSOC));
    }
