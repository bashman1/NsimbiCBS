...
try {
    $date = date('Y-m-d');
    $date_of_last_pay = null;
    // $date = '2023-06-01';
    $db_handler = new DbHandler();

    $data = json_decode(file_get_contents("php://input"), true) ?? $_REQUEST;

    $loans = $db_handler->database->fetchAll('SELECT * FROM `loan` WHERE `branchid` =\'' . $data['branch'] . '\' AND auto_status=0 AND `status` IN (?) ORDER BY loan_no LIMIT 1000', loan_active_statuses());
    ...
    foreach ($loans as $loan) {
        ...
        try {
            ...
            if (count($schedules) > 0) {
                ...
                // return;
                if (@$first_schedule['principal_paid'] > 0 || @$first_schedule['interest_paid'] > 0) {
                    $date_of_last_pay = db_date_format(@$first_schedule['date_of_payment']);
                }
                ...
                $db_handler->update('loan', [
                    ...
                    'arrearsbegindate' => @$next_due_date,
