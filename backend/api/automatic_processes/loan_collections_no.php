...
try {
    ...
    $loans = $db_handler->database->fetchAll('SELECT * FROM `loan` WHERE `branchid`=\'' . $data['branch'] . '\' AND (principal_due>0 OR interest_due>0) AND  `status` IN (?) ', loan_active_statuses());
    ...
    foreach ($loans as $loan) {
        ...
        if ($wallet_balance > 0) {
            ...
            /**
             * update loan dues to be all in arrears immediately
             */
            $db_handler->update(
                'loan',
                [
