...
// $item->send_sms = $data->send_sms;


try {
    $result = $item->createDeposit();

    // insert into the audit trail table -- for creating a client

    // generate and organise audit trail info
    $audit_info  = array(
        "action" => $data->pay_method . '  Deposit',
        "log_desc" => 'Created New Deposit for: - ' . $data->client . ' of UGX: ' . number_format($data->amount ?? 0) . ': Record Date: ' . $data->date,
        "uid" => $data->user,
        "branch" => $data->branch,
        "bank" => NULL,
        "ip" => '',
...
*/
    ...
    if ($sms_consent) {
        ...
        $smstype = $item2->getBankSMStypeStatus($data->branch, 'on_deposit');
        ...
        if ($smstype != 0 && $smstype['s_status'] == 1) {
            ...
            if ($sms_bal > $sms_price || $sms_bal == $sms_price) {
                ...
                $sms = $item2->decryptSMS($smstype['temp_body'], 'on_deposit', $trxnDetails);
