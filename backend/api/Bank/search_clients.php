...
if ($itemCount > 0) {
    ...
    foreach ($stmt as $r) {
        ...
        $return_string .= "<tr><td><img class='rounded-circle' width='30'
             ...
             <td><a href='client_profile_page.php?id=" . encrypt_data($r['userId']) . "' title='View Client's profile' class='load_via_ajax'> " .($r['firstName'] ? strtoupper($r['firstName'] . " " . $r['lastName']) : strtoupper($r['shared_name']) )." : NIN - " . $r['nin'] . "</a></td> 
        ...
        $return_string .= '<td class="text-center no_print"><a class="btn btn-sm btn-primary load_supplement_ajax" href="add_deposit.php?t=' . encrypt_data($r['userId']) . '">Deposit</a></td></tr>';
