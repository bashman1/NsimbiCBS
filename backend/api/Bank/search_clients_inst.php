...
if ($_GET['bank'] == '') {
    $stmt = $item->getBranchClientsInst(preg_replace("/\s+/", "", $_GET['term']));
} else {
    $stmt = $item->getBankClientsFees(preg_replace("/\s+/", "", $_GET['term']));
}
...
if ($itemCount > 0) {
    ...
    foreach ($stmt as $r) {
        ...
        $return_string .= '<td class="text-center no_print"><a class="btn btn-sm btn-primary load_supplement_ajax" href="pay_school_fees.php?t=' . encrypt_data($r['userId']) . '">Pay School Fees  </a></td></tr>';
