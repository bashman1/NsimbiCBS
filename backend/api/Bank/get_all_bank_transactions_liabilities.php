...
if ($_GET['branch'] == '') {
    $item->id = $_GET['bank'];
    $stmt = $item->getAllBankTransactionsLiabilities();
} else {

    $item->id = $_GET['branch'];
    $stmt = $item->getAllBranchTransactionsLiabilities();
}
...
if ($itemCount > 0) {
    ...
    while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
        ...
        $u = array(
            ...
            "dr" =>  $cr_dr == 'debit' ? '<span class="text-danger">' . number_format($amount) . '</span>' : 0,
            "dr_actual" =>  $cr_dr == 'debit' ? $amount : 0,
            "cr_actual" => $cr_dr == 'credit' ? $amount : 0,
            "ref_no" => '<span class="no_print clickable_ref_no" ref-no="' . $ref . '" tid="' . $tid . '">' . $ref . '</span>',
            "cr" => $cr_dr == 'credit' ? '<span class="text-danger">' . number_format($amount) . '</span>' : 0,
