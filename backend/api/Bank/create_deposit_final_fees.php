...
$item->description = $data->reason . ' ( ' . $data->pay_method . ' ) for - ' . $data->sno . ' - ' . $data->sname . ' - Class:' . $data->sclass . ' - Term:' . $data->term ;
...
$item->parent_phone = $data->deposited_phone;
$item->parent_name = $data->deposited;
$item->send_sms_parent = $data->send_sms;
$item->send_sms_school = $data->send_sms_school;
$item->external_ref = '0';
...
try {
    ...
    $audit_info  = array(
        "action" => $data->pay_method . '  School Fees Payment',
        "log_desc" => 'Created New School Fees Payment for: - SNO: ' . $data->sno . ' ' . $data->client . ' of UGX: ' . number_format($data->amount ?? 0) . ': Record Date: ' . $data->date,
        "uid" => $data->user,
        "branch" => $data->branch,
        "bank" => NULL,
        "ip" => '',
            ...
            if ($smstype != 0 && $smstype['s_status'] == 1) {
                ...
                if ($sms_bal > $sms_price || $sms_bal == $sms_price) {
                    ...
                    $trxnDetails  = array(
                        ...
                        "sno" => $data->sno ?? '',
                        "sname" => $data->sname ?? '',
                        "pname" => $data->deposited ?? '',
                        "pcontact" => $data->deposited_phone ?? '',
                        "sclass" => $data->sclass ?? '',
                    );
                    ...
                    if ($phone) {
                        ...
                        if ($res = 'OK') {
                            ...
                            foreach ($phone as $value) {
                                ...
                                $item->insertSMSOutBox($value, $sms, $senderid, (int)$data->client ?? 0, (int)$my_charge, 'sent', 1, $data->branch, 'Deposit Trxn SMS');
                            }
                        } else {
                            // get charge per sms
                            $my_charge = $smstype['charge'] / count($phone);
        ...
        if ($data->send_sms_school) {
            ...
            if ($smstype != 0 && $smstype['s_status'] == 1) {
                ...
                if ($sms_bal > $sms_price || $sms_bal == $sms_price) {
                    ...
                    // get client's primary phone number
                    $phone = $item->getClientPhone($data->client);
