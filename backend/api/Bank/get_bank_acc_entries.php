...
$stmt = $item->getBankRangeTransactions();

$cash_acc_details = $item->getAccountNameBalance($data->staff);
$bf = $item->getBankAccountBf($data->staff, $data->start);
...
if ($itemCount > 0) {
    ...
    while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
        ...
        if ($meth != 'cash' || $t_type == 'AJE' || $t_type == 'STB' || $t_type == 'BTS' || $t_type == 'BTB' || $t_type == 'CAP') {
            $u = array(
                "_did" => $tid,
                "bf" => $bf,
                "dr_acid" => $dr_acid ?? '',
