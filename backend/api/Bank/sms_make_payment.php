...
$sql = $item->ac_deposit_funds($data->phone, $data->amount, 'SMS PURCHASE: ' . $data->reason);
...
if ($sql["Status"] == "OK") {
    ...
    $stmt = $itemn->InsertMmTrxnLogSMS($data->acid, $data->acid, $data->id, isset($sql['TransactionReference']) ? $sql['TransactionReference'] : "", 'SMS PURCHASE: ' . $data->reason, $stcode == "0" ? "SUCCEEDED" : ($stcode == "1" ? "PENDING" : "FAILED"), $data->amount, $mes, $data->branch, $data->phone, $ext_ref,$data->bank,'SMS');
} else {
    if ($sql['StatusCode'] == "--") {
