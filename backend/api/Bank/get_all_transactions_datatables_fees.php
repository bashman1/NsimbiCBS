...
try {
    if ($transaction->branchId) {
        if ($transaction->transaction_type == "D") {
            $recordsTotal = count($transaction->getAllBranchDepositsFees()->fetchAll(PDO::FETCH_ASSOC));
        }
    } else {
        if ($transaction->transaction_type == "D") {
            $recordsTotal = count($transaction->getAllBankDepositsFees()->fetchAll(PDO::FETCH_ASSOC));
        }
    }

    $records = $transaction->getBankTransactionsDatatablesFees();
