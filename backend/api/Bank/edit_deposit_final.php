...
try {
    $result = $item->editDeposit();

    // insert into the audit trail table -- for creating a client

    // generate and organise audit trail info
    $audit_info  = array(
        "action" => $data->pay_method . '  Deposit Update',
        "log_desc" => 'Updated Deposit TID: ' . $data->tid . ' for: - ' . $data->client . ' of UGX: ' . number_format($data->amount ?? 0) . ': Record Date: ' . $data->date,
        "uid" => $data->user,
        "branch" => $data->branch,
        "bank" => NULL,
        "ip" => '',
    ...
    if ($result === true) {
        ...
        echo $ApiResponser::SuccessMessage("Deposit Updated successfully !");
    } else {
