            ...
            if ($collates != '') {
                ...
                foreach ($collates as $row) { ?>
                    <tr>
                        ...
                        <td><?= normal_date_short($row['date_of_payment']); ?></td>
                        <td><?= number_format($row['principal']) ?></td>
                        <td><?= number_format($row['interest'])  ?></td>
                        <td><?= number_format($row['amount']) ?></td>
                        <td><?= number_format($row['balance']) ?></td>
                        <td><?= number_format($details[0]['loan']['forced_saving']) ?></td>
                        <td><strong><?= number_format($details[0]['loan']['forced_saving'] + $row['amount']) ?></strong></td>
                    </tr>
                ...
                <?php
                    ...
                    $bal = $bal + $row['amount'];
                    $bal_s = $bal_s + $details[0]['loan']['forced_saving'];
                    $bal_sa = $bal_sa + $details[0]['loan']['forced_saving'] + $row['amount'];
                } ?>
                <tr>
                    ...
                    <td></td>
                    <td><strong><?= number_format($bal_s) ?></strong></td>
                    <td><strong><?= number_format($bal_sa) ?></strong></td>
                </tr>
            <?php
            } ?>
        </tbody>
