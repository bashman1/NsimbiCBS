...
<script>
    ...
    $(document).ready(function() {
        ...
        $(document).on("click", '.clickable_ref_no_shares', function(e) {
            e.preventDefault();
            handle_share_ref_click_options($(this));
        });
    ...
    function handle_share_ref_click_options(item) {
        var ref_no = item.attr('ref-no');
        var tid = item.attr('tid');
        $(".transaction-custom-menu").remove();
        ...
        var options = "<ul class='transaction-custom-menu'>" +
            "<li class='dropdown-header dropdown-item'>-- " + ref_no + " --</li>" +
            "<li data-action='view' ref-no = '" + ref_no + "' tid = '" + tid + "' class='dropdown-item'><a href='trash_share_trxn.php?tid=" + tid + "'>Delete Entry</a></li>" +
            // "<li data-action='edit' ref-no = '"+ref_no+"'>Edit Transaction</li>"+
            // "<li data-action='delete' ref-no = '"+ref_no+"'>Delete Transaction</li>"+
            "</ul>";
...
<script>
    ...
    function h_print_divImages(div) {
        setTimeout(function() {
            $("#" + div).printThis({
                importCSS: true,
                importStyle: true,
                loadCSS: "",
                copyTagClasses: true, // Copies classes from the original tags
                base: true, // Ensures the base tag is used for relative URLs
            });
        }, 500); // Delay to ensure images load
    }
