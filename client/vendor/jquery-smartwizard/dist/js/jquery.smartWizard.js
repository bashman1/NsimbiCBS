  ...
  var SmartWizard = /*#__PURE__*/function () {
    ...
    _createClass(SmartWizard, [{
      ...
      value: function _showNext() {
        ...
        var sections = $('ul.nav.nav-wizard');
        var selected = null;
        var sectionId = null;
        sections.each(async function (index, ele) {
          console.log("index ", index);
          if (current_index == index) {
            sectionId = $(this).find('a').attr('href').replace(/#/, '');
          }
        });

        // console.log("selection Id", sectionId);
        var currentWizard = $(`#${sectionId}`);
        var attributes = currentWizard.find('input,select');
        var is_valid = true;
        attributes.each(function () {
          $(this).parent('div').find('span.input-error').remove();
          $(this).removeClass('invalid-input');
          if ($(this).attr('required') && $(this).val() == '') {
            $(this).addClass('invalid-input');
            var error_label = $(this).parent('div').find('label').text().replace(/\*/, '');
            $(this).parent('div').append(`<span class="input-error text-danger"> ${error_label} is required </span>`);
            is_valid = false;
          }
        });
