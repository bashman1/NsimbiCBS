...
$title = 'FIXED SAVINGS LEDGER REPORT';


require_once('includes/response.php');
require_once('includes/ReportService.php');
$ReportService = new ReportService();
$request_data = ['ReportModel' => 'TransactionReport', 'ReportModelMethod' => 'getfdsLedgerReport', 'is_expense_report' => false, 'branchId' => @$_GET['branch'], 'loan_product_id' => 0];
                            ...
                            <tbody>
                                ...
                                ?>
                                    <tr>
                                        ...
                                        <td class=""><a href="fixed_deposit_details.php?id=<?= @$record['fd_id'] ?>"><?= @$record['fd_id'] ?></a></td>
                                        <td> <?= @$record['ac_no'].' : '. (@$record['shared_name'] ? @$record['shared_name'] : (@$record['firstName'] . ' ' . @$record['lastName'])) ?> </td>
                                        <td> <?= normal_date_short(@$record['fd_date']) ?> </td>

                                        <td> <?= number_format(@$record['fd_amount'] ?? 0) ?> </td>
                                        <td> <?= @$record['fd_duration'] . strtoupper(@$record['duration_type']) ?> </td>
                                        <td> <?= @$record['fd_status'] > 0 ? 'CLOSED' : 'RUNNING' ?> </td>



                                    </tr>
                                <?php
                                    $total_ac += (int) $record['fd_amount'];
                                } ?>

                                <tr>
                                    <th colspan="3">Totals (<?= number_format($i) ?>) </th>
