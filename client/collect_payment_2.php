...
if (isset($_POST['submit'])) {
    $principal = amount_to_integer($_POST['principal']);
    $interest = amount_to_integer($_POST['interest']);
    $ac_bal = amount_to_integer($_POST['ac_bal']);
    if (($principal + $interest) <= 0) {
        setSessionMessage(false, "Both Principal & Interest Paid can not be 0 or less than 0");
        // header('location: loan_details_page?id=' . $_POST['lno']);
        RedirectCurrent();
    } else if (($ac_bal < ($principal + $interest)) && $_POST['pay_method'] == 'saving') {
        setSessionMessage(false, "Insufficient funds on savings account");
        // header('location: loan_details_page?id=' . $_POST['lno']);
        RedirectCurrent();
    } else {
        $res = $response->createLoanRepayPI($_POST);
        // var_dump($res);
        // exit;
        if ($res['success']) {
...
<body>
                        ...
                        <div class="card">
                            <div class="card-header">
                                <h4 class="card-title">
                                    ...
                                    Manual Loan Repayment Form (Principal & Interest Specified)
                                </h4>

                            </div>
                            <div class="card-body">
                                <p class="text-muted mb-3">
                                    Using the form below, you can add a loan Repayment to a customer's loan statement
                                </p>
                                ...
                                <div class="btc-price">
                                    ...
                                    <div class="row">
                                        ...
                                        <div class="col-lg-2">
                                            <span class="text-muted">Savings Balance (UGX)</span>
                                            <h6 class="mt-0">
                                    ...
                                    <div class="row">
                                        <div class="col-lg-3">
                                            <span class="text-muted">Interest Balance (UGX)</span>
                                            <h6 class="mt-0"><?= number_format(@$selected_loan['loan']['interest_balance']??0) ?></h6>
                                        </div>
                                        <div class="col-lg-2">
                                            <span class="text-muted">Interest Due (UGX)</span>
                                            <h6 class="mt-0"><?= number_format(@$selected_loan['loan']['interest_due']) ?></h6>
                                        </div>
                                        <div class="col-lg-3">
                                            <span class="text-muted">Principal Balance (UGX)</span>
                                            <h6 class="mt-0"><?= number_format(@$selected_loan['loan']['principal_balance']) ?></h6>
                                        </div>
                                        <div class="col-lg-2">
                                        ...
                                        <div class="col-lg-2">
                                            <span class="text-muted">Penalty Balance (UGX)</span>
                                            <h6 class="mt-0">
                                                <?= number_format(@$selected_loan['loan']['penalty_balance']) ?></h6>
                                        </div>
                                    </div>
                                    <hr class="hr-dashed">
                                </div><br />
                                ...
                                <?php if ($total_loan_balance > 0) { ?>
                                    <div class="basic-form">
                                        <form method="POST" id="LoanPaymentForm">
                                            ...
                                            <div class="mb-3">
                                                <label class="col-form-label pt-0" for="principal"> Principal (in
                                                    Ugx)</label><br>

                                                <input type="text" class="form-control comma_separated" name="principal" value="<?= @$selected_loan['loan']['principal_due'] ?>" id="principal" data-max="<?= @$selected_loan['loan']['principal_balance'] ?>" required>
                                            </div>
                                            ...
                                            <div class="mb-3">
                                                ...
                                                <input type="text" class="form-control comma_separated" name="interest" value="<?= @$selected_loan['loan']['interest_due'] ?>" id="interest" data-max="<?= $selected_loan['loan']['interest_balance'] ?>" required>
                                            </div>
                                            ...
                                            <input type="tex" class="form-control" name="interest" value="" id="interest" max="" required data-type="amount">
                                                ...
                                                <div class="mb-3">
                                                    <label class="col-form-label pt-0" for="amount_tot">Total Amount Paid (in
                                                        Ugx)</label><br>
                                                    <input type="text" class="form-control" value="<?= number_format(@$selected_loan['loan']['interest_due'] + @$selected_loan['loan']['principal_due']) ?>" id="amount_tot" readonly>
                                                </div>
    ...
    ***********************************-->
        ...
        <script>
            $('body').on('keyup', 'input#principal', function() {
                let form = $("#LoanPaymentForm");
                let actual_loan_balance = $("#actual_loan_balance").val() || 0
                // let amount = $("#amount").val() || 0
                let interest_paid = $("#interest").val() || 0
                let principal_paid = $("#principal").val() || 0
                let interest = 0

                principal_paid = principal_paid.replace(/\,/g, ''); // 1125, but a string, so convert it to number
                principal_paid = parseInt(principal_paid, 10);

                interest_paid = interest_paid.replace(/\,/g, ''); // 1125, but a string, so convert it to number
                interest_paid = parseInt(interest_paid, 10);

                // interest = interest.replace(/\,/g, ''); // 1125, but a string, so convert it to number
                interest = parseInt(interest, 10);

                let loan_balance = actual_loan_balance - principal_paid - interest_paid;
                let amount_val = principal_paid + interest_paid;

                loan_balance = loan_balance.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",");
                amount_val = amount_val.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",");
                $("#balance").val(loan_balance);
                $("#amount_tot").val(amount_val);

            })

            $('body').on('keyup', 'input#interest', function() {
                let form = $("#LoanPaymentForm");
                let actual_loan_balance = $("#actual_loan_balance").val() || 0
                // let amount = $("#amount").val() || 0
                let interest_paid = $("#interest").val() || 0
