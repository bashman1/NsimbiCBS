            ...
            <div class="container-fluid">
                ...
                <div class="card">
                    <div class="card-header">
                        ...
                        <?php if ($permissions->hasSubPermissions('create_client')) { ?>
                            <div class="btn-group" role="group">
                                <button type="button" class="btn btn-primary dropdown-toggle" data-bs-toggle="dropdown" aria-expanded="false">Add New Client</button>
                                <div class="dropdown-menu" style="margin: 0px;">
                                    <a class="dropdown-item" href="add_individual_client.php?1">Member</a>
                                    <a class="dropdown-item" href="add_individual_client.php?2">Non-Member</a>
                                </div>
                            </div>
                        <?php } ?>
    ...
    ***********************************-->
    ...
    <script type="text/javascript">
        ...
        function bindtoDatatable() {
            ...
            var table = $('#clients_datatable').dataTable({
                ...
                ajax: {
                    ...
                    url: `<?= BACKEND_BASE_URL ?>User/get_all_bank_clients_datatables.php?bank=<?= $user[0]['bankId'] ?>&branch=<?= $user[0]['bankId'] == '' ? $user[0]['branchId'] : '' ?>&branchId=<?= @$_REQUEST['branch'] ?>&client_type=<?= @$_REQUEST['client_type'] ?>&gender=<?= @$_REQUEST['gender'] ?>&actype=<?= @$_REQUEST['actype'] ?>&start_date=<?= @$_REQUEST['start_date'] ?>&end_date=<?= @$_REQUEST['end_date'] ?>&client_type_section=individual`,
                    ...
                    dataSrc: function(response) {
                        ...
                        for (let record of data) {
                            ...
                            datatable_data.push({
                                'image': `<img class="rounded-circle" width="35" src="https://ucscucbs.net/${record.profilePhoto}"  onerror="this.onerror=null; this.src='images/account.png';" alt="">`,
                                ...
                                'oldmem': `<a class="text-primary" href='client_profile_page.php?id=${encrypt_data(record.userId)}'>${record.old_membership_no??'' }</a>`,
                                'actype': record.actype ? 'Member' : 'Non-Member',
                                'save_name': record.save_name ?? '',
                                'name': `<a class="text-primary" href='client_profile_page.php?id=${encrypt_data(record.userId)}'>${record.firstName} ${record.lastName ? record.lastName : ''}</a> `,
                                    ...
                                    <div class="dropdown custom-dropdown mb-0">
                                        ...
                                        <div class="dropdown-menu dropdown-menu-end">
                                            <a class="dropdown-item"
                                                href="client_profile.php?id=${encrypt_data(record.userId)}"> <i class="fa fa-eye"></i> View Account Profile </a>
                                        </div>
                                    </div>
                                `,
                            })
        ...
        function exportClientsToExcel() {
            ...
            fetch('https://app.ucscucbs.net/backend/api/Bank/export_clients.php?branch=<?= $user[0]['branchId'] ?>&bank=<?= $user[0]['bankId'] ?>&type=&product=&st=&end=')
        ...
        //     a.href = '';
        //     a.download = 'exported_clients_data.xlsx';
